          ...
          <div class="col">
            <q-tab-panels
              ...
              <q-tab-panel name="torrents">
                ...
                <q-input outlined
                         v-model="clientSettings['download-dir']"
                         label="Downloads directory" />
                <div class="row">
                  <q-checkbox  v-model="clientSettings['start-added-torrents']"
                               label="Start when added" />
                  <q-checkbox  v-model="clientSettings['rename-partial-files']"
                               label="Append .part to incomplete files" />
                </div>
                <div class="text-h5 text-center q-mb-md">Seeding</div>
                <q-checkbox  v-model="clientSettings['seedRatioLimited']"
                             label="Stop seeding at ratio" />
                <q-input outlined
                         v-model="clientSettings['seedRatioLimit']"
                         label="seed ratio"
                         :disable="!clientSettings['seedRatioLimited']"
                         />
                <q-checkbox  v-model="clientSettings['idle-seeding-limit-enabled']"
                             label="stop seeding if idle for" />
                <q-input outlined
                         v-model="clientSettings['idle-seeding-limit']"
                         label="minutes"
                         :disable="!clientSettings['idle-seeding-limit-enabled']"
                         />
              </q-tab-panel>
...
export default {
  ...
  beforeCreate() {
    this.transmissionService = new TransmissionService({store: this.$store});
    this.transmissionService.getClientSettings()
      .then(response => this.clientSettings.setSettings(this.settings))
